//
//  TBPreferentialInformationView.m
//  Telecom
//
...
@interface TBPreferentialInformationView()<TBPreferentialListViewDelegate>
...
@end
@implementation TBPreferentialInformationView
{
...
- (void)setUpViews
{
    ...
    self.amountView = [[TBPreferentialListView alloc] initPreferentialType:PreferentialListTypeAmount isCard:NO];
    self.amountView.delegate = self;
    [scrollView addSubview:self.amountView];
    
    self.rulesView = [[TBPreferentialListView alloc] initPreferentialType:PreferentialListTypeRules isCard:NO];
    self.rulesView.delegate = self;
    [scrollView addSubview:self.rulesView];
    
    self.numberView = [[TBPreferentialListView alloc] initPreferentialType:PreferentialListTypeNumber isCard:NO];
    self.numberView.delegate = self;
    [scrollView addSubview:self.numberView];
    
    self.limitView = [[TBPreferentialListView alloc] initPreferentialType:PreferentialListTypeLimit isCard:NO];
    ...
    self.statusView = [[TBPreferentialStatusView alloc] initPromptString:@"是否发布抵用券"];
...
- (void)theAmountSize:(NSDictionary *__nonnull)amount;
{
    ...
    {
        self.parameter[@"selfmoney"] = number;
...
- (void)updataData:(TBMakingListMode *)makingList;
{
    ...
    if (makingList.preferentialDic.count>0)
    {
        
        NSString *money = [makingList.preferentialDic valueForKey:@"money"];
        NSString *selfmoney = [makingList.preferentialDic valueForKey:@"selfmoney"];
        NSString *num = [makingList.preferentialDic valueForKey:@"num"];
        NSString *ucondit = [makingList.preferentialDic valueForKey:@"ucondit"];
        NSString *sdate = [makingList.preferentialDic valueForKey:@"sdate"];
        NSString *edate = [makingList.preferentialDic valueForKey:@"edate"];
        NSString *ID = [makingList.preferentialDic valueForKey:@"id"];
        ...
        if (money.doubleValue == 0.0f && selfmoney.doubleValue == 0.0f && num.integerValue == 0)
        {
...
- (void)updataMaking;
{
    self.makingList.preferentialDic = self.parameter;
