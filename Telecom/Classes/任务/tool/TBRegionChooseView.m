...
- (UIView *)pickerView:(UIPickerView *)pickerView viewForRow:(NSInteger)row forComponent:(NSInteger)component reusingView:(UIView *)view {
    ...
    if (!pickerCell) {
        pickerCell = [[ZKPickerViewCell alloc] initWithFrame:(CGRect){CGPointZero, pickerView.frame.size.width/3-10, 60} ];
    }
...
- (void)senderClick
{
    ...
    if ([self.delegate respondsToSelector:@selector(selectedProvincesComponent:)]) {
        [self.delegate selectedProvincesComponent:ChooseViewComponentMake([_picker selectedRowInComponent:0], [_picker selectedRowInComponent:1], [_picker selectedRowInComponent:2])];
    }
    [self backClick];
}
