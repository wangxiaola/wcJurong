...
{
    self.backImageNameArray = @[@[@{@"name":@"农家乐",@"image":@"njl-icon"},@{@"name":@"休闲娱乐",@"image":@"xxyl-icon"},@{@"name":@"特色美食",@"image":@"tsms-icon"},@{@"name":@"酒店客栈",@"image":@"jdkz-icon"},@{@"name":@"景区景点",@"image":@"jqjd-icon"},@{@"name":@"服务场所",@"image":@"fwcs-icon"}],
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    [cell setBackImageData:data[indexPath.row]];
    
    if ([indexPath isEqual:[NSIndexPath indexPathForRow:0 inSection:1]]) {
        
        [cell setTheRedDotNumber:self.completeNumber cellSection:1];
    }
    else if ([indexPath isEqual:[NSIndexPath indexPathForRow:1 inSection:1]])
    {
        [cell setTheRedDotNumber:self.imperfectNumber cellSection:1];
    }
    else
    {
        [cell removeRedDot];
...
{
    UICollectionReusableView *headerRV = [collectionView dequeueReusableSupplementaryViewOfKind:kind withReuseIdentifier:@"ReusableView" forIndexPath:indexPath];
    headerRV.backgroundColor = RGB(244, 244, 244);
    return headerRV;
...
- (CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout*)collectionViewLayout sizeForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    {
        cellWidth = _SCREEN_WIDTH/3;
        CGSize size = {cellWidth, cellWidth*0.8};
        return size;
    }
    else
    {
        cellWidth = _SCREEN_WIDTH/4;
...
-(CGSize)collectionView:(UICollectionView*)collectionView layout:(UICollectionViewLayout*)collectionViewLayout referenceSizeForFooterInSection:(NSInteger)section
{
    if(section == 0)
    {
        CGSize size = {_SCREEN_WIDTH, 10.0f};
        return size;
    }
    else
    {
        CGSize size = {_SCREEN_WIDTH, 50.0f};
        return size;
    }
}
