...
- (void)pushTemplateData:(TBPackageData *)data template:(TBTaskListRoot *)root
{
    ...
    {
        TBServicePlaceViewController *vc =[[TBServicePlaceViewController alloc] init];
        if (root)
        {
            vc.merchantsID = root.ID;
            vc.servicePlaceType = TBServicePlaceTypeMaking;
        }
        else
        {
            vc.servicePlaceType = TBServicePlaceTypeNewAdded;
        }
        vc.modelsID = data.ID;
